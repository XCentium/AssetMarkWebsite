...
namespace Salesforce.Common
{
    public class ServiceHttpClient : IServiceHttpClient, IDisposable
    {
            ...
            try
            {
                do
                {
                    ...
                    if (responseMessage.IsSuccessStatusCode)
                    {
                        ...
                        var jToken = jObject.Value<string>(nodeName);

                        //next = (jObject.GetValue("nextRecordsUrl") != null) ? jObject.GetValue("nextRecordsUrl").ToString() : null;
                        next = (jObject.Value<string>("nextRecordsUrl") != null) ? jObject.Value<string>("nextRecordsUrl").ToString() : null;
        ...
        public async Task<T> HttpPostMultiPartFormDataAsync<T>(List<MultipartFormDataObject> list, string urlSuffix, string boundary = null)
        {
            var url = Common.FormatUrl(urlSuffix, _instanceUrl, _apiVersion);
            if (string.IsNullOrWhiteSpace(boundary))
            {
                boundary = "POST---MultiPartFormData---" + DateTime.Now.ToString(System.Globalization.CultureInfo.InvariantCulture);
            }

            MultipartFormDataContent multipartFormDataContent = new MultipartFormDataContent(boundary);
            
            foreach (var multipartObject in list)
            {
                multipartObject.ContentBuilder.BuildHttpContent(multipartFormDataContent);
            }

            var responseMessage = await _httpClient.PostAsync(new Uri(url), multipartFormDataContent).ConfigureAwait(false);
            var response = await responseMessage.Content.ReadAsStringAsync().ConfigureAwait(false);
