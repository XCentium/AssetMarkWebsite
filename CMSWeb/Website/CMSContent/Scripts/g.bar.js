	...
	for (var i = 0; i < len; i++) {
		...
		for (var j = 0; j < (multi || 1); j++) {
			var h = Math.round((multi ? values[j][i] : values[i]) * Y),
                ...
                // bar = this.g.finger(Math.round(X + barwidth / 2), top + h, bardwidth, h, true, type).attr({ stroke: "none", fill: colors[multi ? j : i] });
				bar = this.g.finger(Math.round(X + barwidth / 2), top + h, 14, h, true, type).attr({ stroke: "none", fill: colors[multi ? j : i] });
	...
	if (!opts.stacked) {
		for (var i = 0; i < len; i++) {
			for (var j = 0; j < (multi || 1); j++) {
				...
				covers.push(cover = this.rect(Math.round(X), y + barvgutter, 20, height - barvgutter).attr(this.g.shim));
