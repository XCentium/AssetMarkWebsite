...
if (s_host.indexOf("www.assetmark.com") != -1) {
    var s_account = "assetpublicsite";
}
else {
    var s_account = "assetpublicdev";
}
...
function s_doPlugins(s) {
    ...
    var splitValues = addressUrl.split("/");

    var hasValue = function (value, index) {
        var response = false;
        if (index === undefined) {
            response = splitValues.indexOf(value) != -1;
        } else if (splitValues.length > index) {
            response = splitValues[index] == value
        }

        return response;
    };

    var getArrayValue = function (index) {
        var response = "";
        if (splitValues.length > index) {
            response = splitValues[index];
        }
        return response;
    }

    if (hasValue("en", 3)) {
        splitValues.splice(3, 1);
    }
    ...
    if (hasValue("en.aspx", 3) || hasValue("", 3)) {
        tagTop = "Home";
        tagLeaf = "Home";
    }
    else if (hasValue("about")) {
        tagTop = "About";

        if (hasValue("news.aspx")) { tagLeaf = "News & Insights" }
        else if (hasValue("news") && hasValue("files")) { tagMiddle = "News"; tagBottom = "Files"; tagLeaf = getArrayValue(6); }
        else if (hasValue("videos.aspx")) { tagLeaf = "Videos" }
        else if (hasValue("careers.aspx")) { tagLeaf = "Careers" }
        else if (hasValue("files")) { tagMiddle = "Files"; tagLeaf = getArrayValue(5) }
    } else if (hasValue("people")) {
        tagTop = "Our People and Our Values";

        if (hasValue("important-things")) { tagLeaf = "What We Believe" }
        else if (hasValue("how-we-started")) { tagLeaf = "About AssetMark" }
        else if (hasValue("our-people")) { tagLeaf = "Our Team" }
    } else if (hasValue("practice")) {
        tagTop = "Practice"

        if (hasValue("our-complete-offering")) {
            tagMiddle = "Our Complete Offering";

            if (hasValue("", 5)) { tagLeaf = "Home" }
            else if (hasValue("platform-and-you-video")) { tagLeaf = "Platform Video" }
        }
        else if (hasValue("lets-talk")) { tagLeaf = "Contact" }
        else if (hasValue("faq")) { tagLeaf = "FAQs" }
        else if (hasValue("advisor-podium")) { tagLeaf = "Testimonials" }
        else if (hasValue("platform-and-you-video")) { tagMiddle = "Our Complete Offering"; tagLeaf = "Platform Video"; alert("entre"); }
    } else if (hasValue("together")) {
        tagTop = "Purpose-Built Investments";

        if (hasValue("investment-philosophy")) {
            tagMiddle = "Investment Philosophy";

            if (hasValue("", 5)) { tagLeaf = "Home" }
            else if (hasValue("asset_allocation")) { tagLeaf = "Asset Allocation Approaches" }
            else if (hasValue("total_support_tool")) {tagLeaf = "Total Support Tool"}
        }
        else if (hasValue("strategists")) { tagLeaf = "Strategists" }
        else if (hasValue("guided-portfolios")) {
            tagMiddle = "Guided Portfolio";

            if (hasValue("", 5)) { tagLeaf = "Home" }
            else if (hasValue("gps")) { tagLeaf = "GPS Solution" }
            else if (hasValue("gps_select")) { tagLeaf = "GPS Select Solution " }
            else if (hasValue("guided_portfolios_video")) { tagLeaf = "Guided Portafolio Video" }
        }
    } else if (hasValue("mystory")) {
        tagTop = "My Story";

        if (hasValue("#matrisian")) { tagLeaf = "Matrisian" }
        else if (hasValue("#disalvo")) { tagLeaf = "Disalvo" }
        else if (hasValue("#clifford")) { tagLeaf = "Clifford" }
        else if (hasValue("myassetmarkstory_1")) { tagLeaf = "My Assetmark Story 1" }
        else if (hasValue("myassetmarkstory_2")) { tagLeaf = "My Assetmark Story 2" }
        else if (hasValue("myassetmarkstory_3")) { tagLeaf = "My Assetmark Story 3" }
    } else if (hasValue("advisorapp")) {
        tagLeaf = "Advisor Ipad App";
    } else if (hasValue("info")) {
        tagTop = "Info";

        if (hasValue("terms")) { tagLeaf = "Terms of Use"; }
        else if (hasValue("privacy")) { tagLeaf = "Privacy and Security Information"; }
        else if (hasValue("funds")) {
            tagMiddle = "GuideMark and GuidePath Funds";

            if (hasValue("", 5)) { tagLeaf = "Home" }
            else if (hasValue("gps-risk-return")) { tagLeaf = "GuideMark/GuidePath Funds XBRL Risk/Return" }
            else if (hasValue("information-statements")) { tagLeaf = "GuideMark and GuidePath Funds Information Statements" }
            else if (hasValue("gm-core")) { tagLeaf = "GuideMarkSM Core Fixed Income Fund" }
        }
        else if (hasValue("aris")) { tagLeaf = "Aris"; }
        else if (hasValue("savos")) { tagLeaf = "Savos"; }
        else if (hasValue("trust")) { tagLeaf = "AssetMark Trust Company"; }
        else if (hasValue("community")) { tagLeaf = "Community"; }
    }
...
function setOmniture(path) {
    var s_host = document.location.hostname;
    var s_account = '';
    if (s_host.indexOf('www.ewealthmanager.com') != -1)
        s_account = 'genfinewm';
    else
