...
{
	internal class DelimitedTextStream
	{
		private TextReader oTextReader;
		private char cDelimeter;

		public DelimitedTextStream(TextReader s) : this(s, ',') { }

		public DelimitedTextStream(TextReader oTextReader, char cDelimeter)
		{
		...
		private string GetNextItem()
		{
			...
			while (true)
			{
				...
				if ((postdata || !quoted) && c == cDelimeter)
					// end of item, return
		...
		private char GetNextChar(bool eat)
		{
			if (pos >= length)
			{
				length = oTextReader.ReadBlock(buffer, 0, buffer.Length);
				if (length == 0)
				{
					EOS = true;
					return '\0';
